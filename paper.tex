...
\usepackage{physics, amsmath, amssymb, esvect, environ}
...
\newcommand{\crosses}[6]{
 \vu{\i}\det\mat{#2}{#3}{#5}{#6} 
-\vu{\j}\det\mat{#1}{#3}{#4}{#6} 
+\vu{ k}\det\mat{#1}{#2}{#4}{#5}
}

\newcommand{\expandcrosses}[6]{
  \vu{\i}\left[ \left( #2 \times #6 \right) - \left( #3 \times #5 \right) \right] 
- \vu{\j}\left[ \left( #1 \times #6 \right) - \left( #3 \times #4 \right) \right]
+ \vu{ k}\left[ \left( #1 \times #5 \right) - \left( #2 \times #4 \right) \right]
}
...
\subsection{Stage 2}
\problem{Find the equation of a sphere if one of its diameters has end points $(2,1,4)$ and $(4,3,10)$.}
\begin{answer}
Let $A = (2,1,4)$ and $B=(4,3,10)$. The centrepoint, $C$, of the circle will lie in the middle of $A$ and $B$, and the radius, $R$, is $\frac{1}{2}|B-A|$.

\begin{align*}
B-A &= \langle 4-2, 3-1, 10-4 \rangle = \langle 2, 2, 6 \rangle \\
d = |B-A| &= \sqrt{2^2 + 2^2 + 6^2} = 2\sqrt{11} \\
r &= \frac{1}{2} d = \frac{1}{2} |B-A| = \sqrt{11} \\
C &= A + \frac{1}{2}(B - A) = (2 + \frac{1}{2} 2, 1 + \frac{1}{2} 2, 4 + \frac{1}{2} 6) \\
 &= (3, 2, 7)
\end{align*}

Therefore, the equation for the sphere would be
\begin{equation*}
...
\problem{Compute the areas of the parallelograms determined by the following vectors.}
\begin{answer}
The area of a parallelogram produced by two-dimensional vectors is eequal to the magnitude of their cross-product\footnote{The vector cross product is inherently defined in three-dimensions, so really this is the magnitude of their three dimensional vector (as the true vector result would exist solely on the $z$-axis.}, which is equal to the determinant of a $2 \times 2$ matrix comprised of both vectors.
\end{answer}
\subproblem{$\langle -3, 1 \rangle, \langle 4, 3 \rangle$}
\begin{answer}
\begin{align*}
|\langle -3, 1 \rangle \cross \langle 4, 3 \rangle|
&= \left| \det \begin{bmatrix} -3 & 1 \\ 4 & 3 \end{bmatrix} \right| \\
&= |(-3 \times 3) - (1 \times 4)| \\ 
&= |-9-4| = 13
\end{align*}
\end{answer}
\subproblem{$\langle 4, 2 \rangle, \langle 6, 8 \rangle$}
\begin{answer}
\begin{align*}
|\langle 4, 2 \rangle \cross \langle 6, 8 \rangle|
&= \left| \det \begin{bmatrix} 4 & 2 \\ 6 & 8 \end{bmatrix} \right| \\
&= |(4 \times 8) - (6 \times 2)| \\
&= |32 - 12| = 20
\end{align*}
\end{answer}
...
\problem{Compute the volumes of the parallelepipeds determined by the following vectors.}
\begin{answer}
The area of a parallelepiped between three vectors $\va{a}, \va{b},$ and $\va{c}$ is
$$|\va{a} \vdot (\va{b} \cross \va{c})|.$$ 
\end{answer}

\subproblem{$\langle 4, 1, -1 \rangle, \langle -1, 5, 2 \rangle, \langle 1, 1, 6 \rangle$}
\begin{answer}
Let $\va{a} = \langle 4, 1, -1 \rangle, \va{b} = \langle -1, 5, 2 \rangle,$ and $\va{c} = \langle 1, 1, 6 \rangle$.

\begin{align*}
\va{b} \cross \va{c} &= \det \crosstable{-1}{5}{2}{1}{1}{6} = \vu{\i} \det\mat{5}{2}{1}{6} - \vu{\j}\det\mat{-1}{2}{1}{6} + \vu{k} \det \mat{-1}{5}{1}{1} \\
&= \vu{\i} (5 \times 6 - 2 \times 1) - \vu{\j} (-1 \times 6 - 2 \times 1) + \vu{k} (-1 \times 1 - 5 \times 1) \\
&= \vu{\i} (30 - 2) - \vu{\j} (-6 -2) + \vu{k}(-1-5) \\
&= 28 \vu{\i} + 8\vu{\j} - 6\vu{k} \\
&= \langle 28, 8, -6 \rangle \\
\va{a} \vdot (\va{b} \cross \va{c}) &= (4 \times 28) + (1 \times 8) + (-1 \times -6) \\
&= 112 + 8 + 6 = 126
\end{align*}
Therefore, the volume is 126 cubic units.
\end{answer}

\subproblem{$\langle -2, 1, 2 \rangle, \langle 3, 1, 2 \rangle, \langle 0, 2, 5 \rangle$}
\begin{answer}
Let $\va{a} = \langle -2, 1, 2 \rangle, \va{b} = \langle 3, 1, 2 \rangle,$ and $\va{c} = \langle 0, 2, 5 \rangle$.
\begin{align*}
\va{b} \cross \va{c} 
&= \det\crosstable{3}{1}{2}{0}{2}{5} 
= \crosses{3}{1}{2}{0}{2}{5} \\
&= \expandcrosses{3}{1}{2}{0}{2}{5} \\
&= \ihat(5 - 4) - \jhat(15) + \khat(6) \\
&= \langle 1, -15, 6 \rangle \\
\va{a} \vdot (\va{b} \cross \va{c})
&= (-2 \times 1) + (1 \times -15) + (2 \times 6) \\
&= -2 -15 + 12 = -5,
\end{align*}
Therefore, the volume is $|-5|=5$ cubic units.
\end{answer}

\problem{Compute the dot product of the vectors $\va{a}$ and $\va{b}$. Find the angle between them.}
\begin{answer}
The angle $\theta$ between vectors can be calculated using the equation
$$\cos\theta = \frac{\va{u} \vdot \va{v}}{|\va{v}||\va{u}|},$$
which subsequently becomes
$$\theta = \arccos \frac{\va{u} \vdot \va{v}}{|\va{v}||\va{u}|} $$
\end{answer}

\subproblem{$\va{a} = \langle 1, 2 \rangle, \va{b} = \langle -2, 3 \rangle$}
\begin{answer}
\begin{align*}
\va{a} \vdot \va{b} &= (1 \times -2) + (2 \times 3) = -2 + 6 = 4 \\
|\va{a}| &= \sqrt{1^2 + 2^2} = \sqrt{5} \\
|\va{b}| &= \sqrt{(-2)^2 + 3^2} = \sqrt{13}
\end{align*}
Now, for the angle:
\begin{align*}
\theta &= \arccos \frac{\va{a} \vdot \va{b}}{|\va{a}| |\va{b}|} = \arccos \frac{4}{\sqrt{5 \times 13}} = \SI{60}{deg}
\end{align*}
\end{answer}

\subproblem{$\va{a} = \langle -1, 1 \rangle, \va{b} = \langle 1, 1 \rangle$}
\begin{answer}
\begin{align*}
\va{a} \vdot \va{b} &= (-1 \times 1) + (1 \times 1) = 0 \\
\therefore \theta &= \SI{0}{deg}.
\end{align*}
\end{answer}

\subproblem{$\va{a} = \langle 1,1 \rangle, \va{b} = \langle 2, 2 \rangle$}
\begin{answer}
\negspace
\begin{align*}
\va{a} \vdot \va{b} &= (1 \times 2) + (1 \times 2) = 2 + 2 = 4 \\
|\va{a}| &= \sqrt{1^2 + 1^2} = \sqrt{2} \\
|\va{b}| &= \sqrt{2^2 + 2^2} = \sqrt{8}.
\end{align*}
Now for the angle:
\begin{align*}
\theta = \arccos \frac{4}{4} = \arccos 1 = \SI{0}{deg}.
\end{align*}
\end{answer}

\subproblem{$\va{a} = \langle 1, 2, 1 \rangle, \va{b} = \langle -1, 1, 1 \rangle$}
\begin{answer}
\begin{align*}
\va{a} \vdot \va{b} 
&= (1 \times -1) + (2 \times 1) + (1 \times 1)
= -1 + 2 + 1 = 2 \\
|\va{a}| &= \sqrt{1^2 + 2^2 + 1^2} = \sqrt{6} \\
|\va{b}| &= \sqrt{(-1)^2 + 1^2 + 1^2} = \sqrt{3}.
\end{align*}
Now, for the angle:
\begin{align*}
\theta = \arccos \frac{2}{\sqrt{18}} = \arccos \frac{1}{\sqrt{3}} \approx \SI{0.955}{rad}/\SI{54.73}{deg}
\end{align*}
\end{answer}

\subproblem{$\va{a} = \langle -1, 2, 3 \rangle, \va{b} = \langle 3, 0, 1 \rangle$}
\begin{answer}
\begin{align*}
\va{a} \vdot \va{b}
&= (-1 \times 3) + (2 \times 0) + (3 \times 1)
= -3 + 3 = 0 \\
\therefore \theta &= \SI{0}{deg}.
\end{align*}
\end{answer}

\problem{Determine the angle between vectors $\va{a}$ and $\va{b}$ if}
\subproblem{$\va{a} = \langle 1, 2 \rangle, \va{b} = \langle 3, 4 \rangle$}
\begin{answer}
\negspace
\begin{equation*}
\theta = \arccos \frac{1 \times 3 + 2 \times 4}{\sqrt{1^2 + 2^2} \sqrt{3^2 + 4^2}} = \arccos \frac{3 + 8}{\sqrt{5} \sqrt{25}} = \arccos \frac{11}{5 \sqrt{5}} \approx \SI{10.3}{deg}
\end{equation*}
\end{answer}

\subproblem{$\va{a} = \langle 2, 1, 4 \rangle, \va{b} = \langle 4, -2, 1 \rangle$}
\begin{answer}
\begin{equation*}
\theta = \arccos \frac{2 \times 4 + 1 \times -2 + 4 \times 1}{\sqrt{2^2 + 1^2 + 4^2} \sqrt{4^2 + (-2)^2 + 1^2}} = \arccos \frac{10}{21} \approx \SI{61.563}{deg}
\end{equation*}
\end{answer}

\subproblem{$\va{a} = \langle 1, -2, 1 \rangle, \va{b} = \langle 3, 1, 0 \rangle$}
\begin{answer}
\begin{equation*}
\theta = \arccos \frac{1 \times 3 - 2 \times 1 + 1 \times 0}{\sqrt{1^2 + (-2)^2 + 1^2} \sqrt{3^2 + 1^2}} = \arccos \frac{1}{2\sqrt{15}} \approx \SI{82.58}{deg}
\end{equation*}
\end{answer}

\problem{Determine all values of $y$ for which the given vectors are perpendicular.}
\begin{answer}
For the two vectors to be perpendicular, their dot product must be 0.
\end{answer}

\subproblem{$\langle 2, 4 \rangle, \langle 2, y \rangle$}
\begin{answer}
$$4 + 4y = 0 \therefore y = -1$$
\end{answer}

\subproblem{$\langle 4, -1 \rangle, \langle y, y^2 \rangle$}
\begin{answer}
$$4y -4y^2 = 0 \therefore y = 0, 1$$
\end{answer}

\subproblem{$\langle 3, 1, 1 \rangle, \langle 2, 5y, y^2 \rangle$}
\begin{answer}
$$6 + 5y + y^2 = 0 \rightarrow (y + 3)(y+2) = 0, \therefore y = -2, -3$$
\end{answer}

\problem{Let $\va{u} = -2\ihat + 5\jhat$ and $\va{v} = \alpha \ihat - 2\jhat$. Find $\alpha$ so that}
\subproblem{$\va{u} \perp \va{v}$}
\begin{answer}
If $\va{u} \perp \va{v}$ then $\va{u} \vdot \va{v} = 0$:
\negspace
\begin{align*}
-2\alpha -10 &= 0 \\
-2(\alpha + 5) &= 0,
...
\subproblem{$\va{u} || \va{v}$}
\begin{answer}
To be parallel, $\vu{u} = t\vu{v}$:
\negspace
\begin{align*}
\langle -2, 5 \rangle &= t \langle \alpha, -2 \rangle
\end{align*}
...
-2 &= \alpha t,
\end{align*}
which is solved by $t = -5/2$, therefore $\alpha = 4/5$.
\end{answer}

\subproblem{The angle between $\va{u}$ and $\va{v}$ is $60$ deg.}
\begin{answer}
\begin{align*}
  \frac{\va{u} \vdot \va{v}}{|\va{u}||\va{v}|} &= \cos \frac{\pi}{3} \\
  \va{u} \vdot \va{v} &= \cos(\frac{\pi}{3}) |\va{u}||\va{v}| \\
  -2(\alpha + 5) &= \cos(\frac{\pi}{3}) \sqrt{29} \sqrt{\alpha^2 + 4} \\
  \frac{\alpha + 5}{\sqrt{a^2 + 4}} &= -\frac{\sqrt{29}}{2} \cos(\frac{\pi}{3}) \\
  \dots
  \alpha = 
\end{align*}
\end{answer}

\problem{Define $\va{a} = \langle 1, 2, 3 \rangle$ and $\va{b} = \langle 4, 10, 6 \rangle$.}
\subproblem{Find the component of $\va{b}$ in the direction of $\va{a}$.}
\subproblem{Find the projection of $\va{b}$ on $\va{a}$.}
\subproblem{Find the projection of $\va{b}$ perpendicular to $\va{a}$.}

\problem{Compute $\langle 1, 2, 3 \rangle \cross \langle 4, 5, 6 \rangle$.}

\problem{Calculate the following cross products.}
\subproblem{$\langle 1, -5, 2 \rangle \cross \langle -2, 1, 5 \rangle$}
\subproblem{$\langle 2, -3, -5 \rangle \cross \langle 4, -2, 7 \rangle$}
\subproblem{$\langle -1, 0, 1 \rangle \cross \langle 0, 4, 5 \rangle$}

\problem{Let $\va{p} = \langle -1, 4, 2 \rangle, \va{q} = \langle 3, 1, -1 \rangle, \va{r} \langle 2, -3, -1 \rangle$. Check, by direct comparison, that}
\subproblem{$\va{p} \cross \va{p} = 0$}
\subproblem{$\va{p} \cross \va{q} = -\va{q} \cross \va{p}$}
\subproblem{$\va{p} \cross (3\va{r}) = 3(\va{p} \cross \va{r})$}
\subproblem{$\va{p} \cross (\va{q} + \va{r}) = \va{p} \cross \va{q} + \va{p} \cross \va{r}$}
\subproblem{$\va{p} \cross (\va{q} + \va{r}) = \va{p} \cross \va{q} + \va{p} \cross \va{r}$}
\subproblem{$\va{p} \cross (\va{q} \cross \va{r}) \ne (\va{p} \cross \va{q}) \cross \va{r}$}

\problem{Calculate the area of the traignel with vertices $(0,0,0), (1,2,3)$, and $(3,2,1)$.}

\problem{A particle $P$ of unit mass whose position in space at time $t$ is $\va{r}(t)$ has angular momentum $L(t) = \va{r}(t) \cross \va{r}'(t)$. If $\va{r}''(t) = \rho(t)\va{r}(t)$ for a scalar function $\rho$, show that $L$ is constant, i.e. does not change with time. Here, $'$ denotes $\dv{t}$.}
